...
class LogisticRegression(model.Model):
	...
	def train(self, data: mldata.ExampleSet, truths: np.array):
		...
		weights = np.random.rand(
			len(npdata))  # randomly initialize weights for each feature
		bias = np.random.rand(len(npdata),
							  1)  # randomly initialize biases? not sure if i
		# should even incorporate biases
		self.weights = self.gradient_descent(npdata, truths, weights,
											 stepsize=1, skip=set())
	...
	def conditional(self, x, y):
		return -y * np.math.log(x) - (1 - y) * np.math.log(
			1 - x)  # calculate log likelihood to maintain convexity instead
	...
	def predict(self, data: mldata.ExampleSet):
		...
		weighted_feats = np.transpose(
			np.array([self.weights[i] * f for i, f in enumerate(ndata)]))
		log_likelihood_scores = np.array([self.sigmoid(sum(w)) for w in
										  weighted_feats])  # vector to save
		# the sigmoid values for each example
	...
	def conditional_log_likelihood(self, labels, sigmoids, weights):
		...
		conditional_ll = (1 / 2) * np.square(
			np.linalg.norm(weights)) + self.cost * (
								 sum(-np.log(pos_sigmoids)) + sum(
							 -np.log(1 - neg_sigmoids)))
	...
	def gradient_descent(self, ndata, truths, weights, stepsize, epsilon=1,
		...
		while not (len(skip) == len(ndata) or iterations > 1000):
			...
			weighted_feats = np.transpose(np.array([weights[i] * f for i, f in
													enumerate(
														ndata)]))  #
			...
			for j in range(len(ndata)):
				if j not in skip:
					gradient[j] = (1 / len(ndata)) * np.sum(
						gradient_func(sum(weighted_feats[:][i]), ndata[j][i],
									  truths[i])
						for i, f in enumerate(ndata[j]))  # i indexes examples

			finished = np.argwhere(abs(
				gradient) < epsilon)  # todo: check which is local minimum.
