...
class LogisticRegression(model.Model):
	...
	def predict(self, data: mldata.ExampleSet, weights, truths):
		...
		sigmoid = np.array(
			[self.sigmoid(sum(w)) for w in weighted_feats])  # vector to save the sigmoid values for each example
		out = self.conditional_log_likelihood(truths, sigmoid, weighted_feats, complexity=1) / len(sigmoid)
