...
class ID3(Model):
	...
	def id3(
		...
		# check base conditions, if we're at max depth or pure
		if mlutil.is_homogeneous(data) or self._at_max_depth(depth):
			# return a leaf node with the
		...
		if len(left_data) == 0:
			# return a leaf of our current majority label
			left_child = node.Node(data=majority_label, parent=parent)
		# set to current majority label
