...
def main() -> NoReturn:
    ...
    try:
        ...
        max_depth = 5
        use_info_gain = 0
    if use_info_gain < 0:  # only relevant for when we're running the experiment
        partition_count = [3, 5, 7, 10]
    else:
        partition_count = [1]
    ...
    for z in partition_count: # run the experiment
        ...
        learner = algorithm.ID3(max_depth=max_depth, split_function=split_criteria, partitions=z)
        runAlg(use_cv, data, learner)


def runAlg(use_cv, data, learner):
    accuracies = []
    tree_sizes = []
    first_feats = []
    depths = []
    if use_cv:
        ...
        for i in range(fold_num):
            ...
            print(f'positive guesses / positive truth labels {sum(p) / sum(mlutil.get_labels(train))}')
            print(p)
            print('\n Average Metrics')
            print('-----------------------')
        print_results(
